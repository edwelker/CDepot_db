...
class DatesVisited(models.Model):
    ...
    sellers = models.ManyToManyField(Seller, through='DateList')
...
class DateList(models.Model):
    ...
    dates_visited = models.ForeignKey(DatesVisited)
    
    
class DateListInline(admin.TabularInline):        
    model = DateList
    extra = 1
